...
export class BookingComponent implements OnInit {
  ...
  constructor(
    ...
    this.bookingForm = this.fb.group({
      ...
      end_date: [moment().add(30, 'day').format('D/M/YYYY'), Validators.required],
    });
  }
  ...
  */
  ...
  async present_date_picker(event, type) {
    ...
    let date = type == 'start_date' ?
      this.bookingForm.get('start_date').value : this.bookingForm.get('end_date').value;
    ...
    type == 'start_date' ?
      this.bookingForm.get('start_date').setValue(
        `${data.data.date.day}/${data.data.date.month}/${data.data.date.year}`
      ) :
      this.bookingForm.get('end_date').setValue(
        `${data.data.date.day}/${data.data.date.month}/${data.data.date.year}`
      )
  }
  ...
  calculate_estimates() {
    ...
    this.total = Number((this.duration / 30) * Number(this.data.peak_rate)).toFixed(2);
