...
export class BillboardComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data) {
      ...
      this.mdplocation = this.data.mdplocation;
      this.mdptype = this.data.mdptype;
      this.billboardForm.setValue({
        width: this.data.screen_width,
        height: this.data.screen_height,
        name: this.data.name,
        ...
        mdplocation: this.data.mdplocation,
        mdptype: this.data.mdptype
      })
      this.load_map();

    } else {
      this.latitude = Number(this.location_services.lat.value);
      this.longitude = Number(this.location_services.lng.value);
      this.load_map();
  ...
  */
  public async load_map() {
    this.map = new google.maps.Map(document.getElementById('map_canvas_billboard'), {
  ...
  async submit() {
    ...
    let mdptype = this.billboardForm.get('mdptype').value;
    let mdplocation = this.billboardForm.get('mdplocation').value;
    let screen_height = Number(this.billboardForm.get('height').value);
    let screen_width = Number(this.billboardForm.get('width').value);
    let date_created = moment().format('DD/MM/yyyy HH:MM:SS');
    let date_modified = moment().format('DD/MM/yyyy HH:MM:SS');
    this.photo_url = await (await this.storage.pushFileToStorage('sitelive/billboards', this.file)).getDownloadURL();
    let orientation = (screen_height == screen_width) ? 'Square'
      : (screen_height > screen_width) ? 'Potrait'
        : (screen_height < screen_width) ? 'Landscape'
          : 'Other';
